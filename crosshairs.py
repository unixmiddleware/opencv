...
while True:
    ...
    if not frame is None:
        ...
        for contour in contours:
            ...
            x2 = x + w//2
            y2 = y + h//2
            print('Vertical',0,y2,dispW,y2)
            print('Horizontal',x2,0,x2,dispH)
            cv.line(frame,(0,y2),(dispW,y2),(0,0,255),3)
            cv.line(frame,(x2,0),(x2,dispH),(0,255,0),3)
            cv.rectangle(frame,(x,y),(x+w,y+h),(255,0,0),3)
        cv.imshow(win,frame)
        cv.moveWindow(win,0,0)
    if cv.waitKey(3000)==ord('q'): break
print(cv.__version__)
cam.release()
